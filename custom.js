...
let startCustomButton;
let customBackButton;
let classicPreset;
let preset1;
let preset2;
let preset3;
...
function PresetWidget(x_, y_, xsize_, ysize_, text_, selected_, subtext_, spacing_) {
  this.text = text_;
  this.x = x_;
  this.y = y_;
  this.xsize = xsize_;
  ...
  this.display = function() {
    ...
    fill(255);
    textAlign(CENTER, CENTER);
    text(this.text, this.x + this.xsize/2, this. y + this.ysize/2 - this.spacing);
    textSize(floor(canvasWidth/40));
    text(this.subtext, this.x + this.xsize/2, this. y + this.ysize/2 + this.spacing);
    textAlign(LEFT, TOP);
  }
