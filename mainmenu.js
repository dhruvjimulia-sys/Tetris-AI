...
function displayMainMenu() {
  ...
  text("Artificial Intelligence learning to play", width/2, height*(48/100));
  text("the world's most influential puzzle game", width/2, height*(54/100));
...
function MenuWidget(x_, y_, xsize_, ysize_, text_, subtext_, spacing_) {
  ...
  this.display = function() {
    ...
    text(this.text, this.x + this.xsize/2, this.y + this.ysize/2 - this.spacing + increment);
    textSize(floor(canvasWidth/40));
    text(this.subtext, this.x + this.xsize/2, this.y + this.ysize/2 + this.spacing + increment);
    textAlign(LEFT, TOP);
  }
}
